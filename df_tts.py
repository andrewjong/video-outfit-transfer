...
import config
from src.df_datasets import TextureDataset
...
parser = argparse.ArgumentParser(
    ...
    description="Train the texture stage FOR DEEP FASHION.",
)
parser.add_argument(
    "-t",
    "--texture_dir",
...
parser.add_argument(
    "--experiment", default=None, help="Name the experiment for output path"
)
parser.add_argument(
    "-o",
    "--out_dir",
...
if args.resume_latest:
    saved_models = os.listdir(os.path.join(args.save_dir, args.experiment))
...
if args.epoch != 0:
    matching_epoch_files = glob(os.path.join(args.save_dir, args.experiment, f"generator_{args.epoch:02d}*"))
    ...
    generator.load_state_dict(
        torch.load(
            os.path.join(
                args.save_dir, args.experiment, f"generator_{start_of_epoch}"
            )
        )
    )
...
texture_dataset = TextureDataset(
    args.texture_dir, args.rois_db, args.clothing_dir, p_hflip=0.75
)
...
for epoch in tqdm(
    ...
    for i, (textures, rois, cloths, targets) in enumerate(batch):
        if cuda:
            textures = textures.cuda()
            rois = rois.cuda()
            cloths = cloths.cuda()
        ...
        loss_f1 = criterion_l1_feat(gen_fakes, targets)
        loss_mlf = criterion_mlf(gen_fakes, targets)

        # Total loss
        loss_texture = loss_l1 + 0.5 * loss_f1 + loss_mlf + loss_adv
